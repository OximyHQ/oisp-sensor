...
pipeline {
    ...
    stages {
        stage('Setup') {
            steps {
                sh 'curl -fsSL https://github.com/oximyHQ/oisp-sensor/releases/latest/download/oisp-sensor-x86_64-unknown-linux-gnu.tar.gz | tar xz && sudo mv oisp-sensor /usr/local/bin/'
            }
        }
