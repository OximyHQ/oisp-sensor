...
fn draw_footer(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let spans: Vec<Span> = tabs.iter().flat_map(|(key, name, active)| {
        let style = if *active {
...
fn draw_timeline(frame: &mut Frame, area: Rect, app: &App) {
    let events: Vec<ListItem> = app.timeline
        .iter()
        .skip(app.scroll)
        ...
        .map(|event| {
            ...
            let proc = envelope.process.as_ref()
                .and_then(|p| p.name.clone())
                .unwrap_or_else(|| "?".to_string());
            
            let detail = match event.as_ref() {
                OispEvent::AiRequest(e) => {
                    let provider = e.data.provider.as_ref()
                        .map(|p| p.name.as_str())
                        .unwrap_or("?");
                    let model = e.data.model.as_ref()
                        .map(|m| m.id.as_str())
                        .unwrap_or("?");
                    format!("{} {}", provider, model)
                }
                OispEvent::AiResponse(e) => {
                    let tokens = e.data.usage.as_ref()
                        .and_then(|u| u.total_tokens)
                        .map(|t| format!("{}tok", t))
                        .unwrap_or_default();
                    let latency = e.data.latency_ms
                        .map(|l| format!("{}ms", l))
                        .unwrap_or_default();
...
fn draw_inventory(frame: &mut Frame, area: Rect, app: &App) {
    ...
    // Providers
    let provider_items: Vec<ListItem> = app.providers.values()
        .map(|p| {
            ListItem::new(Line::from(vec![
                Span::styled(format!("{:<15}", p.name), Style::default().fg(Color::Cyan)),
                Span::styled(format!("{:>6} req  ", p.request_count), Style::default().fg(Color::White)),
    ...
    // Apps
    let app_items: Vec<ListItem> = app.apps.values()
        .map(|a| {
            ...
            ListItem::new(Line::from(vec![
                Span::styled(format!("{:<15}", a.name), Style::default().fg(Color::Magenta)),
                Span::styled(format!("{:>6} req  ", a.request_count), Style::default().fg(Color::White)),
...
fn draw_traces(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let items: Vec<ListItem> = traces.iter()
        .take(area.height as usize - 2)
        .map(|trace| {
            ...
            ListItem::new(Line::from(vec![
                Span::styled(
                    format!("{} ", status),
                    Style::default().fg(if trace.is_complete { Color::Green } else { Color::Yellow })
                ),
                Span::styled(
